...
ws.on('open', () => {
  ...
  ws.send('CONNECT\naccept-version:1.2\nhost:/\n\n\0');
  // Topics for table 1 (order 36 belongs to table 1)
  ws.send('SUBSCRIBE\nid:sub-1\ndestination:/topic/table/1/orders\n\n\0');
  ws.send('SUBSCRIBE\nid:sub-2\ndestination:/topic/waiters/alerts\n\n\0');
  ws.send('SUBSCRIBE\nid:sub-3\ndestination:/topic/kitchen/orders\n\n\0');
  console.log('Subscribed. Waiting 2s...');
  
  setTimeout(async () => {
    console.log('--- Patching order 36 IN_PREPARATION ---');
    await patch('/api/orders/36/status', {status: 'IN_PREPARATION'});
    
    console.log('--- Patching order 36 READY ---');
    await patch('/api/orders/36/status', {status: 'READY'});
    
    console.log('--- Calling waiter for table 1 ---');
    await post('/api/orders/sessions/8ca87f0a-de15-4677-a359-f784380d0072/call-waiter', {});
    
    console.log('HTTP calls done. Waiting 5s...');
  }, 2000);
});
